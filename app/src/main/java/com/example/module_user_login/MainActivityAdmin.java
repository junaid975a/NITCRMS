...
import com.example.module_user_login.fragments.AddItems;
import com.example.module_user_login.fragments.AdminDashboard;
import com.example.module_user_login.fragments.AdminReallocationRequest;
import com.example.module_user_login.fragments.AdminViewComplaints;
import com.example.module_user_login.fragments.AdminViewRequests;
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if(!user.getImageURL().equals("default")) {
                    try {
                        ...
                        Glide.with(MainActivityAdmin.this).load(user.getImageURL()).into(drawer_header_img);
                    } catch (Exception e) {
        ...
        navigationView.setNavigationItemSelectedListener(item -> {
            ...
            if(id == R.id.admin_dashboard) {
                Objects.requireNonNull(getSupportActionBar()).setTitle("Dashboard");
                loadFragment(new AdminDashboard());
            } else if(id == R.id.admin_add_items){
                //Toolbar toolbar = findViewById(R.id.my_toolbar);
                //setSupportActionBar(toolbar);
                Objects.requireNonNull(getSupportActionBar()).setTitle("Add New Items");
                //getSupportActionBar().setDisplayHomeAsUpEnabled(true);
                loadFragment(new AddItems());
            } else if(id == R.id.admin_view_requests) {
                Objects.requireNonNull(getSupportActionBar()).setTitle("View Requests");
                loadFragment(new AdminViewRequests());
            } else if(id == R.id.admin_view_complaints) {
                Objects.requireNonNull(getSupportActionBar()).setTitle("View Complaints");
                loadFragment(new AdminViewComplaints());
            } else if(id == R.id.admin_profile) {
                Objects.requireNonNull(getSupportActionBar()).setTitle("Profile");
                loadFragment(new UserProfile());
            }  else if(id == R.id.admin_view_reallocation_request){
                Objects.requireNonNull(getSupportActionBar()).setTitle("View Reallocation Requests");
                loadFragment(new AdminReallocationRequest());
            }else if(id == R.id.admin_logout) {
                FirebaseAuth auth = FirebaseAuth.getInstance();
                auth.signOut();
                Intent intent = new Intent(MainActivityAdmin.this, UserFirstPage.class);
